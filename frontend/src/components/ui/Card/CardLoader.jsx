/**
 * KisanO Design System — Card Package
 * CardLoader
 *
 * The CardLoader component renders a loading skeleton with pulse or shimmer
 * animations to indicate that card content is being loaded. It supports
 * multiple row types (text, image, avatar, custom) and can be customized
 * with row count and size.
 *
 * Single Responsibility: This component only renders loading skeletons.
 * It does not handle Card layout, content, or other sections.
 *
 * @module components/ui/Card/CardLoader
 */

import { forwardRef, memo, useMemo } from 'react';
import PropTypes from 'prop-types';
import { motion, AnimatePresence, useReducedMotion } from 'framer-motion';

import {
  CARD_SIZES,
  
} from './cardVariants';
import {
  mergeClasses,
 
} from './cardUtils';

/* ---------------------------------- */
/* Component‑specific tokens          */
/* ---------------------------------- */

/** Height presets for skeleton rows, keyed by size and shape. */
const SKELETON_HEIGHTS = {
  text: {
    xs: 'h-3',
    sm: 'h-3.5',
    md: 'h-4',
    lg: 'h-5',
    xl: 'h-6',
  },
  heading: {
    xs: 'h-4',
    sm: 'h-5',
    md: 'h-6',
    lg: 'h-7',
    xl: 'h-8',
  },
  image: {
    xs: 'h-20',
    sm: 'h-28',
    md: 'h-36',
    lg: 'h-44',
    xl: 'h-52',
  },
  avatar: {
    xs: 'h-8 w-8',
    sm: 'h-10 w-10',
    md: 'h-12 w-12',
    lg: 'h-14 w-14',
    xl: 'h-16 w-16',
  },
};

/** Width presets for text rows (random‑like widths). */
const TEXT_WIDTHS = ['w-3/4', 'w-full', 'w-5/6', 'w-2/3', 'w-1/2'];

/** Default size when no prop is provided. */
const DEFAULT_SIZE = 'md';

/** Default number of rows. */
const DEFAULT_ROWS=3;

/**
 * Determines the border-radius class for a skeleton row.
 * @param {string} shape - 'square', 'rounded', or 'circle'.
 * @returns {string} Tailwind rounded class.
 */
function getSkeletonRadius(shape) {
  switch (shape) {
    case 'circle':
      return 'rounded-full';
    case 'square':
      return 'rounded-none';
    case 'rounded':
    default:
      return 'rounded-md';
  }
}

/* ---------------------------------- */
/* Component                          */
/* ---------------------------------- */

/**
 * CardLoader – a skeleton loader with pulse or shimmer animations.
 *
 * Supports text, heading, image, and avatar rows. The number of rows can be
 * specified as a number (all rows of the same type) or an array of shape
 * objects for granular control.
 *
 * @component
 * @example
 * // Three text rows with pulse animation
 * <CardLoader rows={3} type="text" />
 *
 * @example
 * // Custom rows with image + text
 * <CardLoader rows={[
 *   { type: 'image', height: 'h-48' },
 *   { type: 'heading' },
 *   { type: 'text', width: 'w-3/4' },
 *   { type: 'text' },
 * ]} />
 */
const CardLoader = memo(
  forwardRef(function CardLoader(
    {
      rows = DEFAULT_ROWS,
      type = 'text',
      size = DEFAULT_SIZE,
      shape = 'rounded',
      animated = true,
      variant = 'shimmer', // 'shimmer' or 'pulse'
      disabled = false,
      className = '',
      'aria-label': ariaLabel = 'Loading content',
      ...rest
    },
    ref,
  ) {
    const prefersReducedMotion = useReducedMotion();

    // Resolve size-specific padding and gap from CARD_SIZES.
    const sizeConfig = useMemo(
      () => CARD_SIZES[size] || CARD_SIZES[DEFAULT_SIZE],
      [size],
    );

    // Determine if animations should be disabled.
    const isAnimated = animated && !prefersReducedMotion && !disabled;

    // Build the row definitions.
    const rowDefs = useMemo(() => {
      if (Array.isArray(rows)) {
        return rows;
      }
      // If rows is a number, generate that many rows of the given type.
      const count = typeof rows === 'number' ? rows : DEFAULT_ROWS;
      return Array.from({ length: count }, () => ({ type }));
    }, [rows, type]);

    // Render a single skeleton row.
    const renderRow = (row, index) => {
      const rowType = row.type || 'text';
      const rowShape = row.shape || shape;
      const rowSize = row.size || size;

      // Determine height/width classes.
      const heightMap = SKELETON_HEIGHTS[rowType];
      const heightClass = row.height || (heightMap ? heightMap[rowSize] : '');
      const widthClass = row.width || (rowType === 'text' ? TEXT_WIDTHS[index % TEXT_WIDTHS.length] : 'w-full');

      // For avatar, use square dimensions; for others, width is full.
      const isAvatar = rowType === 'avatar';
      const sizeClasses = isAvatar
        ? heightClass // avatar height includes width (w-* h-*)
        : mergeClasses(heightClass, widthClass);

      const radiusClass = getSkeletonRadius(rowShape);

      // Base skeleton classes.
      const skeletonBase = mergeClasses(
        'bg-gray-200',
        radiusClass,
        sizeClasses,
        isAnimated && variant === 'pulse' && 'animate-pulse',
        row.className || '',
      );

      // Shimmer overlay: a pseudo‑element that moves across the skeleton.
      // We'll render a motion.div with gradient and animate background position.
      const shimmerElement = isAnimated && variant === 'shimmer' ? (
        <motion.div
          className="absolute inset-0 overflow-hidden rounded-inherit"
          initial={{ x: '-100%' }}
          animate={{ x: '100%' }}
          transition={{
            duration: 1.5,
            repeat: Infinity,
            ease: 'ease-in-out',
          }}
        >
          <div
            className="h-full w-1/2 bg-gradient-to-r from-transparent via-white/50 to-transparent"
            style={{ borderRadius: 'inherit' }}
          />
        </motion.div>
      ) : null;

      return (
        <motion.div
          key={index}
          className="relative"
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          exit={{ opacity: 0, scale: 0.95 }}
          transition={{ duration: 0.2, delay: index * 0.05 }}
        >
          <div className={skeletonBase} />
          {shimmerElement}
        </motion.div>
      );
    };

    // Container classes: use sizeConfig padding and gap.
    const containerClasses = useMemo(
      () =>
        mergeClasses(
          'flex flex-col w-full',
          sizeConfig.gap || 'gap-4',
          sizeConfig.padding || 'p-4',
          disabled && 'opacity-50 cursor-not-allowed',
          className,
        ),
      [sizeConfig, disabled, className],
    );

    // Accessibility props.
    const ariaProps = {
      role: 'status',
      'aria-live': 'polite',
      'aria-busy': true,
      'aria-label': ariaLabel,
      'aria-disabled': disabled || undefined,
    };

    return (
      <AnimatePresence initial={false}>
        <motion.div
          ref={ref}
          className={containerClasses}
          {...ariaProps}
          {...rest}
        >
          {rowDefs.map((row, index) => renderRow(row, index))}
        </motion.div>
      </AnimatePresence>
    );
  }),
);

CardLoader.displayName = 'CardLoader';

CardLoader.propTypes = {
  /**
   * Number of rows or an array of row definition objects.
   * Each row object can have: `type` (text, heading, image, avatar),
   * `height` (custom Tailwind height class), `width` (custom width class),
   * `shape` (square, rounded, circle), `className`.
   */
  rows: PropTypes.oneOfType([
    PropTypes.number,
    PropTypes.arrayOf(
      PropTypes.shape({
        type: PropTypes.oneOf(['text', 'heading', 'image', 'avatar']),
        height: PropTypes.string,
        width: PropTypes.string,
        shape: PropTypes.oneOf(['square', 'rounded', 'circle']),
        className: PropTypes.string,
      }),
    ),
  ]),
  /** Default row type when `rows` is a number. */
  type: PropTypes.oneOf(['text', 'heading', 'image', 'avatar']),
  /** Size preset – controls padding, gap, and row heights. */
  size: PropTypes.oneOf(['xs', 'sm', 'md', 'lg', 'xl']),
  /** Default shape for rows. */
  shape: PropTypes.oneOf(['square', 'rounded', 'circle']),
  /** Whether to animate the skeletons (pulse or shimmer). */
  animated: PropTypes.bool,
  /** Animation variant: 'shimmer' (sliding gradient) or 'pulse' (opacity pulse). */
  variant: PropTypes.oneOf(['shimmer', 'pulse']),
  /** Disables the loader (dim and remove animations). */
  disabled: PropTypes.bool,
  /** Additional CSS classes. */
  className: PropTypes.string,
  /** Accessible label for the loading region. */
  'aria-label': PropTypes.string,
};

export default CardLoader;